...
// ONE BASED INDEXING
vector<int> segTree(2*(1e5+5),inf);

void build(int start,int end,int ind,vector<int>& a){
  if(start==end){
    segTree[ind] = a[start];
    ...
    build(start,mid,2*ind,a);
    build(mid+1,end,2*ind+1,a);
...
void update(int start,int end,int ind,int val,int segInd,vector<int>& a){
  if(start==end){
    a[ind] = val;
    segTree[segInd] = val;
  }
    ...
    if(ind>=start && ind<=mid){
      update(start,mid,ind,val,2*segInd,a);
    }
    else{
      update(mid+1,end,ind,val,2*segInd+1,a);
    }

    segTree[segInd] = min(segTree[2*segInd],segTree[2*segInd+1]);
  }
  
}


int query(int start,int end,int segInd,int l,int r,vector<int>& a){
  if(r<start || l>end){
    return inf;
  }
  ...
  if(l<=start && end<=r){
    ...
    int mid = (end-start)/2+start;
    int t1 = query(start,mid,2*segInd,l,r,a);
    int t2 = query(mid+1,end,2*segInd+1,l,r,a);
 ...
 while(q--){
  ...
  cin>>c>>l>>r;
    ...
    if(c=='q'){
       l--;
        r--;
      cout<<query(0,n-1,1,l,r,a)<<nline;
    }
    else{
      l--;
      update(0,n-1,l,r,1,a);
    }
 }
} 
